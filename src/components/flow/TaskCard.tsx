...
import { Member, generateInitials, getMemberProperty } from '../../utils/helpers';
...
//        channel.publish("plan-it", { type:'node-title-change',node: JSON.stringify({id:data.id,editedTitle:editedTitle}) });
        setEditing(false);
    ...
    //const selfInCell = self?.location?.row === data.id
    ...
    //    console.log("here",labelColor)
    // const memberName = selfInCell
    //     ? "You"
    //     : getMemberProperty(cellMembers as Member[], "name");
    ...
    // const onChange = useCallback((evt: { target: { value: any; }; }, id: number) => {
    ...
    const updateSelection = useCallback((id: number) => {
        //console.log("selected", id, update);
